...
def classifying_positive_negative(a):
    ...
    for i in results:
        words_without_links = [word for word in i['tweet'][0].split() if (('https' not in word) and ('@' not in word) and ('http' not in word) and ((word == 'not') or (word not in stopwords_set))) ]
        temp = ' '.join(words_without_links)
        temp = temp.strip()
        if(len(temp)>100): 
                     
            blob = TextBlob(temp)
            polarity.append([i['tweet'][0],blob.sentiment.polarity,blob.sentiment.subjectivity,len(temp)])   

    df = pd.DataFrame(data = polarity,columns = ['Tweet','Polarity','Subjectivity','Length'])   
   # print(df.head())
